...
describe('[Header]', () => {
    ...
    describe('[Error Handling]', () => {
        it('Should throw an error when no parameters were provided', async () => {
            ...
            expect(result).toContain('<!-- Error occurred in the Header component: dataUrl is not supported -->');
        });
    ...
    describe('[Main Function]', () => {
        ...
        it('Should show the story details when on a story page', async () => {
            const mockPageData = {
                id: '178076',
                short_name: 'Test Story',
                metadata: {
                    storyLayout: true,
                    srContentType: 'Story',
                    srContentMainTopic: [{ id: 123, name: 'Research' }]
                }
            };

            const fetchCalls = vi.fn()
                .mockResolvedValueOnce(mockedFetch) // First call returns site data
                .mockResolvedValueOnce(mockPageData); // Second call returns page data
        ...
        it('Should NOT show the story details when on a normal page', async () => {
            const mockPageData = {
                id: '178076',
                short_name: 'Test Story',
                metadata: {
        ...
        it('Should contain an h1 when on a story page', async () => {
            const mockPageData = {
                id: '178076',
                short_name: 'Test Story',
                metadata: {
        ...
        it('Does not contain an H1 on a normal page', async () => {
            const mockPageData = {
                id: '178076',
                short_name: 'Test Story',
                metadata: {
        ...
        it('Search form actions go to the search page', async () => {
            const mockSiteDataWithResultPage = {
                ...mockedFetch,
                search: {
                    ...mockedFetch.search,
                    resultPage: 'https://news.stanford.edu/search'
                }
            };
            ...
            const fetchCalls = vi.fn()
                .mockResolvedValueOnce(mockSiteDataWithResultPage) // First call returns site data
                .mockResolvedValueOnce(mockPageData); // Second call returns page data
            ...
            // Count occurrences of the search resultPage URL in the HTML
            const matches = result.match(/action='https:\/\/news\.stanford\.edu\/search'/g);
    ...
    describe('[Edge Cases]', () => {
        it('Should throw error when fetch for header data will fail', async () => {
            ...
            await expect(main(defaultMockData, defaultMockInfo)).rejects.toThrow(`Error occurred in the Header component: Error parsing header data JSON response: Network Error`);
        });
        ...
        it('Should throw error when fetch for header data will not return object', async () => {
            ...
            await expect(main(defaultMockData, defaultMockInfo)).rejects.toThrow(`Error occurred in the Header component: Error parsing header data JSON response: Invalid API response: siteData is missing or not an object.`);
        });
        ...
        it('Should handle site being undefined in siteData', async () => {
            ...
            await expect(main({ dataUrl: 'https://example.com/json' }, defaultMockInfo)).rejects.toThrow(`Cannot destructure property 'url' of 'site' as it is undefined.`);
        });
